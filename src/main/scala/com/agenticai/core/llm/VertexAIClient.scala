 ...
 *
 * This is a simplified mock implementation that doesn't require the Google Cloud dependencies.
 * For the actual implementation, see the version in the core module.
 */
class VertexAIClient(config: VertexAIConfig) {
  ...
  /**
   * Stream a completion token by token from the model
   * This mock implementation splits the mock response into tokens
   */
...
object VertexAIClient {
  ...
  /**
   * For actual implementation, please use the VertexAIClient from the core module:
