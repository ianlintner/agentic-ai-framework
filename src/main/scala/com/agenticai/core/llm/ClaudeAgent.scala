...
/** Agent for interacting with Claude via Vertex AI Provides memory-based context management and
...
class ClaudeAgent(
  ...
  /** Process user input and generate a response Stores conversation in memory for context
    *
    * @param input
    *   User input text to process
    * @param timestampOpt
    *   Optional timestamp for testing; defaults to current time
    */
  def process(
      input: String,
    ...
    for
      ...
      responseTimestampOpt = timestampOpt.map(ts =>
        Instant.ofEpochMilli(ts.toEpochMilli + 100)
      ) // Add small offset for response
      _ <- ZStream.fromZIO(saveAssistantMessage(response, responseTimestampOpt))
    yield response

  /** Stream a response for the given input Same as process but with more explicit naming for
    * streaming
    *
    * @param input
    *   User input text to process
  ...
  /** Save an assistant message to memory
    ...
    *   Optional timestamp for testing; defaults to current time
    */
  def saveAssistantMessage(
      message: String,
      timestampOpt: Option[Instant] = None
  ): ZIO[Any, Throwable, Unit] =
    if message.trim.isEmpty then ZIO.unit
    else
      for
        timestamp <- timestampOpt match
          case Some(ts) => ZIO.succeed(ts)
  ...
  def getConversationHistory: ZIO[Any, Throwable, List[ConversationTurn]] =
    ZIO
      .foldLeft(conversationCells.keys.toList)(List.empty[ConversationTurn]) { (acc, cellName) =>
  ...
  /** Format the conversation history as a prompt for Claude Uses the standard Claude conversation
    * format
    */
  ...
  /** Truncate conversation history to the most recent turns A turn is defined as a complete
...
object ClaudeAgent:
  ...
  def make(
    ...
    for memory <-
        if memorySystem != null then ZIO.succeed(memorySystem) else ZIO.service[MemorySystem]
