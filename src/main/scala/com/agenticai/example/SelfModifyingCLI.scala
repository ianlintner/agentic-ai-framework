...
object SelfModifyingCLI extends ZIOAppDefault:
  ...
  case class CLIState(
      ...
      memory: MemorySystem = new InMemorySystem(),
  ...
  def runCLI(state: CLIState): ZIO[Any, Throwable, Unit] =
    for
      ...
      _        <- if command == Command.Exit then ZIO.unit else runCLI(newState)
    yield ()
  ...
  def parseCommand(input: String): Command =
    input.trim.toLowerCase match
      ...
      case s if s.startsWith("modify ") =>
        ...
        if parts.length == 2 then Command.Modify(parts(0), parts(1))
        else Command.Help
    ...
    command match
      case Command.Help =>
        for
          ...
          _ <- Console.printLine(
            "  modify <file> <instruction> - Modify a file based on instruction"
          )
            ...
            if file.exists() then
              for
                ...
                _ <-
                  ...
                  else Console.printLine("Modification cancelled")
              yield ()
            else Console.printLine(s"File not found: $filePath")
        yield state
      ...
      case Command.Exit =>
        for _ <- Console.printLine("Goodbye!")
        yield state
