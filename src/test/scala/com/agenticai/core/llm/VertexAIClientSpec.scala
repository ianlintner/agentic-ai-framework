  ...
  def spec = suite("VertexAIClient")(
    ...
    suite("Basic Functionality")(
      test("should handle completion requests with mock") {
        ...
        for result <- client.complete("Hello")
      ...
      test("should handle streaming with mock") {
        ...
        for
          ...
          containsHello =
            var found = false
            for element <- results do if element.contains("Hello") then found = true
            found
        yield
    ...
    suite("Live API")(
      test("should complete with real API") {
        ...
        yield assertTrue(
          if projectId == "test-project" then result.isLeft
          else result.isRight && result.toOption.get.nonEmpty
        )
      }
    ) @@ (if isLiveTest then identity else ignore)
