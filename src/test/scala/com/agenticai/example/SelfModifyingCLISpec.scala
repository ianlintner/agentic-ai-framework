        ...
        for {
          ...
          dirAndFile <- ZIO.acquireRelease(
            ZIO.attemptBlocking {
              ...
              if (!success) {
                throw new RuntimeException(s"Failed to create test directory at ${testDir.getAbsolutePath}")
              }
              // Create initial test file
              Files.write(testFile.toPath, testContent.getBytes)
              // Verify the file exists and has content
              if (!Files.exists(testFile.toPath) || Files.size(testFile.toPath) == 0) {
                throw new RuntimeException(s"Failed to write test content to ${testFile.getAbsolutePath}")
              }
          ...
          _ <- ZIO.fail(new RuntimeException(s"No backup files found in ${dir.getAbsolutePath}")).when(backupFiles.isEmpty)
