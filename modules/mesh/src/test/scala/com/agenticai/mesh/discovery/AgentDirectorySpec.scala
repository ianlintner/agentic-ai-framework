  ...
  def spec = suite("AgentDirectory")(
    test("register and discover agents") {
      for
        ...
        extractionAgents <- directory.discoverAgents(
          TypedAgentQuery(capabilities = Set("extraction"))
        )

        // Discover agents by multiple capabilities
        translationAgents <- directory.discoverAgents(
          TypedAgentQuery(capabilities = Set("translation"))
        )
        recognitionAgents <- directory.discoverAgents(
          TypedAgentQuery(capabilities = Set("entity-recognition"))
        )

        // Discover agents by property
        englishAgents <- directory.discoverAgents(
          TypedAgentQuery(properties = Map("language" -> "English"))
        )

        // Get specific agent info
        agent1Info       <- directory.getAgentInfo(agentId1)
        nonExistentAgent <- directory.getAgentInfo(UUID.randomUUID())
      yield assert(allAgents.size)(equalTo(3)) &&
      ...
      assert(agent1Info.exists(_.metadata.capabilities.contains("translation")))(
        Assertion.isTrue
      ) &&
    ...
    test("update agent status") {
      ...
      yield assert(initialInfo.exists(_.status == AgentStatus.Active))(Assertion.isTrue) &&
    ...
    test("unregister agent") {
      for
        ...
        allAgents <- directory.getAllAgents()
      yield assert(beforeUnregister.isDefined)(Assertion.isTrue) &&
