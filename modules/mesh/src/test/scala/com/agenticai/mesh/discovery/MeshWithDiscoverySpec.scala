  ...
  def spec = suite("MeshWithDiscovery")(
    test("should register agent with capabilities") {
      for
        ...
        bothAgents <- meshWithDiscovery.findAgentsByCapabilities(
          Set("text-processing", "uppercase")
        )
        noneAgents <- meshWithDiscovery.findAgentsByCapabilities(Set("translation"))
      yield assert(textAgents.size)(equalTo(1)) &&
    ...
    test("should get agent by capabilities") {
      ...
      yield assert(maybeAgent.isDefined)(isTrue) &&
    ...
    test("should find agent by ID") {
      ...
      yield assert(maybeAgentInfo.isDefined)(isTrue) &&
    ...
    test("should update agent status") {
      ...
      yield assert(initialInfo.isDefined)(isTrue) &&
    ...
    test("should subscribe to directory events") {
      ...
      yield assert(agentInfo.isDefined)(isTrue) &&
