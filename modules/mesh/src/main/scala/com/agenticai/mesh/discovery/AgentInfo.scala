 ...
 * @param ref Remote reference to the agent
 * @param metadata Metadata describing the agent's capabilities
 * @param status Current status of the agent
 * @param registeredAt Time when the agent was registered
 * @param lastUpdated Time of the last status update
 * @param loadFactor Optional load factor indicating agent utilization (0.0-1.0)
 */
case class AgentInfo(
  agentId: UUID,
  ref: RemoteAgentRef[_, _],
   ...
   * @return True if the agent has the capability
   */
