...
/** JSON-based implementation of serialization for agent protocol.
  *
  * This implementation uses JSON for serializing and deserializing agents, inputs, and outputs. It
  * provides a simple, human-readable format for development and debugging.
  */
class JsonSerialization extends Serialization:
  ...
  override def getTypeName(clazz: Class[?]): String =
    clazz.getName
  ...
  /** Deserialize an agent.
    */
  def deserializeAgent[I, O](
      bytes: Array[Byte]
  )(implicit tagI: ClassTag[I], tagO: ClassTag[O]): Task[Agent[I, O]] =
