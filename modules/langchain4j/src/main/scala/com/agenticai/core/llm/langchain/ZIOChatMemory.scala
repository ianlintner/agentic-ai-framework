...
/** A ZIO wrapper for Langchain4j's chat memory. This trait provides methods for managing
...
trait ZIOChatMemory:
  /** Adds a user message to the conversation history.
    ...
    *   The user's message
    * @return
    *   A ZIO effect that completes when the message is added
    */
...
object ZIOChatMemory:
  ...
  /** Creates a new in-memory chat memory.
    ...
    * @return
    *   A ZIO effect that completes with a new ZIOChatMemory
    */
  ...
  /** Creates a ZLayer that provides an in-memory ZIOChatMemory.
    ...
    *   The maximum number of messages to store
    * @return
    *   A ZLayer that provides a ZIOChatMemory
    */
