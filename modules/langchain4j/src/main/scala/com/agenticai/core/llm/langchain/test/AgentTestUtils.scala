...
object AgentTestUtils:
  ...
  /** Creates a mock agent for testing.
    ...
    *   The default response for the mock agent
    * @param name
    *   A name for the agent
    * @param maxHistory
    *   The maximum number of messages to keep in history
    * @return
    *   A ZIO effect that resolves to an Agent
    */
  def createMockAgent(
  ...
  /** Utility method for testing an agent with a single interaction.
    ...
    *   The default response for the mock agent
    * @return
    *   A ZIO effect that completes with the agent's response
    */
  ...
  /** Creates a multi-turn conversation test with a mock agent. This is useful for testing context
    * retention across multiple turns.
    *
    * @param testName
    *   The name of the test
    * @param interactions
    *   A list of (input, expectedOutput) pairs
    * @return
    *   A ZIO effect that resolves to a boolean indicating success or failure
    */
  ...
  /** Tests memory truncation in an agent. This verifies that the agent correctly truncates
    * conversation history when it exceeds maxHistory.
    *
    * @param testName
    *   The name of the test
    * @param inputs
    *   A large number of inputs to process sequentially
    * @param maxHistory
    *   The maximum number of turns to keep
    * @return
    *   A ZIO effect that resolves to a boolean indicating success or failure
    */
