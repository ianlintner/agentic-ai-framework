...
/**
 * Tests for the ZIOChatModelFactory.
 * This tests the factory methods for creating different LLM models.
 */
  ...
  def spec = suite("ZIOChatModelFactory")(
    ...
    suite("OpenAI Model Factory")(
      test("should create OpenAI model with default parameters") {
        ...
        val program = for {
          model <- ZIOChatModelFactory.makeOpenAIModel("mock-api-key")
        } yield model
      ...
      test("should create OpenAI model with custom parameters") {
        // This is a compilation check since we don't want to make real API calls
        val program = for {
          model <- ZIOChatModelFactory.makeOpenAIModel(
            apiKey = "mock-api-key",
