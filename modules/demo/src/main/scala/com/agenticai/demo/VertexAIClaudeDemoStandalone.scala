...
/** A completely standalone demo for Claude 3.7 via Vertex AI This file has no dependencies on other
  * parts of the framework and can be used to test basic connectivity
  */
case class Config(
    projectId: String,
...
object VertexAIClaudeDemoStandalone extends ZIOAppDefault:
  ...
  private def generateText(
    ...
    ZIO.attempt {
      val modelEndpoint =
        s"projects/${config.projectId}/locations/us-central1/publishers/${config.publisher}/models/${config.modelName}"
      ...
      val instanceValue = Value
        ...
        .setStructValue(
          Struct
            ...
            .putFields(
              "max_tokens",
              Value.newBuilder().setNumberValue(config.maxOutputTokens).build()
            )
      ...
      response.getPredictionsList.asScala
        .map(_.getStructValue.getFieldsMap.get("text").getStringValue)
        .mkString
    }
