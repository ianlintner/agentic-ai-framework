...
object FactorioCircuitDemo {
  ...
  def runBitPackingDemo(): Unit = {
    ...
    println(s"Bit widths: ${advancedBitWidths.mkString(", ")} (using minimum bits needed for each value)")
    
    BitPacking.packInts(advancedValues, advancedBitWidths) match {
      case Right(packed) => 
        println(s"Packed value: $packed (${packed.toBinaryString} in binary)")
        println(s"Total bits used: ${advancedBitWidths.sum} (compared to ${advancedValues.size * 32} bits for separate integers)")
