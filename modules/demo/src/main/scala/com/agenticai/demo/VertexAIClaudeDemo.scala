 ...
 * 
 * This demo showcases a functional approach to integrating with Vertex AI
 * for Claude 3.7 using ZIO effects for pure functional programming.
 */
object VertexAIClaudeDemo extends ZIOAppDefault:
  ...
  val program: ZIO[Any, Throwable, Unit] = for
    ...
    _ <- ZStream.fromIterable(prompts)
...
class VertexAIClient(config: VertexAIClaudeDemo.VertexAIConfig):
  ...
  /**
   * Generate text from Claude 3.7 using Vertex AI
   * Uses ZIO for proper functional effect handling
   */
  def generateText(prompt: String): ZIO[Any, Throwable, String] = 
    ...
    for
      ...
      response = s"""As an agentic AI framework built with Scala 3 and ZIO, I would help developers create:
