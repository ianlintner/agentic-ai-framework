...
object WorkflowServer extends ZIOAppDefault {
  ...
  def run = {
    ...
    for {
      ...
      _ <- Console.printLine("This demo shows how workflows connect specialized agents to perform a complex task.")
      _ <- Console.printLine("In a web implementation, you would be able to visually build these workflows.")
      _ <- Console.printLine("")
      ...
      _ <- ZIO.foreach(sampleWorkflow.connections) { conn =>
        val sourceNode = sampleWorkflow.nodes.find(_.id == conn.sourceNodeId).map(_.label).getOrElse("Unknown")
        val targetNode = sampleWorkflow.nodes.find(_.id == conn.targetNodeId).map(_.label).getOrElse("Unknown")
      ...
      engine <- ZIO.succeed(new WorkflowEngine(textTransformer, textSplitter, summarizer, buildAgent))
      ...
      _ <- Console.printLine("2. Add various agent types (transformers, summarizers, splitters, build agents, etc.)")
      ...
      _ <- Console.printLine("")
      _ <- Console.printLine("The workflow demo now demonstrates a complete end-to-end process from")
