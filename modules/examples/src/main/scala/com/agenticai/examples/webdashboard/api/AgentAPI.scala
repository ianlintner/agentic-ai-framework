...
object JsonCodecs:
  ...
  // SubtaskResult codec
  implicit val subtaskResultEncoder: JsonEncoder[SubtaskResult] =
    DeriveJsonEncoder.gen[SubtaskResult]

  implicit val subtaskResultDecoder: JsonDecoder[SubtaskResult] =
    DeriveJsonDecoder.gen[SubtaskResult]
...
/** Mock HTTP API for interacting with our agent framework (simplified version for the example
  * without HTTP server)
  */
object AgentAPI extends ZIOAppDefault:
  ...
  def processTaskRequest(jsonRequest: String): ZIO[Any, Throwable, String] =
    for
      ...
      taskRequest <- ZIO
        .fromEither(jsonRequest.fromJson[TaskRequest])
