...
/** Simplified example demonstrating agent discovery concepts.
  *
  * This is a standalone example that shows agent discovery principles without relying on the full
  * mesh implementation. It implements a basic in-memory directory system that enables
...
object SimpleAgentDiscoveryExample extends ZIOAppDefault:
  ...
  class SentimentAnalysisAgent extends Agent[String, SentimentResult]:
    ...
    def process(input: String): Task[SentimentResult] = ZIO.succeed {
      ...
      val sentiment =
        if score < -0.3 then "Negative"
        else if score > 0.3 then "Positive"
        else "Neutral"
  ...
  class SummarizationAgent extends Agent[SummarizationRequest, String]:
    ...
    def process(input: SummarizationRequest): Task[String] = ZIO.succeed {
      ...
      val summary =
        if words.length > length then words.take(length).mkString(" ") + "..."
        else text
    ...
    def process(input: String): Task[String] =
      if input.startsWith("translate:") then
        ...
        for
          ...
          result <- translationAgent.agent
            .asInstanceOf[Agent[TranslationRequest, String]]
            .process(request)
        yield result
      else if input.startsWith("sentiment:") then
        ...
        for
          ...
          result <- summaryAgent.agent
            .asInstanceOf[Agent[SummarizationRequest, String]]
            .process(request)
        yield s"Summary: $result"
      else if input.startsWith("extract:") then
        ...
        for
          ...
          result <- extractionAgent.agent
            .asInstanceOf[Agent[String, Map[String, String]]]
            .process(text)
          formatted = result.map { case (k, v) => s"$k: $v" }.mkString("\n")
        yield s"Extracted entities:\n$formatted"
      else
        ZIO.succeed(
          "Unknown command. Try starting with 'translate:', 'sentiment:', 'summarize:', or 'extract:'."
        )
  ...
  def run: Task[Unit] =
    for
      ...
      _ <- ZIO.foreach(nlpAgents) { info =>
        Console.printLine(
          s"  - Agent: ${info.agentId}, Capabilities: ${info.metadata.capabilities.mkString(", ")}"
        )
      }
