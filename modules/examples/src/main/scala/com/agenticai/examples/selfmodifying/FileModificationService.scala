...
class FileModificationService {
  ...
  private def createBackup(file: File): ZIO[Any, Throwable, File] = {
    for {
      timestamp <- ZIO.succeed(LocalDateTime.now.format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss")))
  ...
  private def validateFilePath(file: File): ZIO[Any, Throwable, Unit] = {
    for {
      ...
      tempDir <- ZIO.attempt(new File(java.lang.System.getProperty("java.io.tmpdir")).getCanonicalFile)
      projectRootPath = projectRoot.getPath
      filePathStr = filePath.getPath
      tempDirPath = tempDir.getPath
      _ <- if (filePathStr.startsWith(projectRootPath) || filePathStr.startsWith(tempDirPath)) {
        ZIO.unit
      } else {
        ZIO.fail(new SecurityException(s"Access denied: $filePath is outside project directory and temp directory"))
      }
    } yield ()
  }
