...
object MemoryManagementExample extends ZIOAppDefault:
  ...
  private val logMemoryMetrics = (metrics: MemoryMetrics) =>
    Console.printLine(s"""
      ...
      |Cells by Tag: ${metrics.cellsByTag
                          .map { case (tag, count) => s"$tag: $count" }
                          .mkString(", ")}
      |Timestamp: ${metrics.timestamp}
      |====================
      |""".stripMargin)

  private val logStatistics = (stats: MemoryStatistics) => Console.printLine(s"""
      |=== Memory Statistics ===
  ...
  val program = for
    ...
    _ <- Console.printLine(
      s"Persistent cleanup removed $persistentCleanupCount cells based on size"
    )
