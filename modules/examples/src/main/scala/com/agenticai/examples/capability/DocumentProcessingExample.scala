 ...
 * 1. Create specialized agents with specific capabilities
 * 2. Register them in an agent directory
 * 3. Discover agents by capability
 * 4. Compose agents into workflows
 * 5. Process documents through the workflows
 */
object DocumentProcessingExample extends ZIOAppDefault {
  ...
  case class SentimentScore(positive: Double, negative: Double, neutral: Double) {
    def dominantSentiment: String = 
      if (positive > negative && positive > neutral) "positive"
      else if (negative > positive && negative > neutral) "negative"
      else "neutral"
  ...
  val sentimentAnalysisAgent = ComposableAgent[String, SentimentScore](
    processImpl = text => {
      ...
      val positiveWords = Set("good", "great", "excellent", "happy", "positive", "wonderful", "amazing")
      val negativeWords = Set("bad", "terrible", "awful", "sad", "negative", "horrible", "disappointing")
  ...
  def run = {
    for {
      ...
      docToSummaryWorkflow <- directory.createWorkflow[Document, String](
        "Document", "String", Set("text-extraction", "summarization")
      )
