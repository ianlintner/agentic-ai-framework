...
/** Example demonstrating the capability-based agent system.
  *
  * This example shows how specialized agents with specific capabilities can be organized,
  * discovered, and composed to create complex workflows.
  */
object ComposableAgentExample extends ZIOAppDefault:
  ...
  val summarizationAgent = ComposableAgent[String, String](
    processImpl = text =>
      ...
      val summary   = if sentences.nonEmpty then sentences(0) + "." else text
      ZIO.succeed(summary)
    ,
  ...
  def run =
    for
      ...
      documents = List(
        ...
        Document(
          "Tech Article",
          "New computer algorithms are revolutionizing software development."
        ),
        Document(
          "Financial News",
          "Banking sector shows strong growth despite economic challenges."
        )
      )
      ...
      _ <- ZIO.foreach(documents) { document =>
        for
          ...
          _ <- Console.printLine(
            s"Document '${document.title}' classified as: ${category.name} (confidence: ${category.confidence})"
          )
        yield ()
      }
      ...
      _ <- ZIO.foreach(documents) { document =>
        for
          ...
          dominant = if sentiment.positive > sentiment.negative then "positive" else "negative"
          _ <- Console.printLine(
            s"Document '${document.title}' sentiment: $dominant (pos: ${sentiment.positive}, neg: ${sentiment.negative})"
          )
        yield ()
      }
      ...
      documentSummarizationWorkflow = docToTextWorkflow.get
        .asInstanceOf[ComposableAgent[Document, String]]
      ...
      _ <- ZIO.foreach(documents) { document =>
        for
          ...
          _ <- Console.printLine(
            s"Auto-discovered workflow result for '${document.title}': $result"
          )
        yield ()
      }
      ...
      parallelAnalysisAgent = ComposableAgent.parallel[String, Any, String](
        ...
        combiner = results =>
          ...
          s"Category: ${category.name} (${category.confidence}), " +
            s"Sentiment: ${
                if sentiment.positive > sentiment.negative then "Positive" else "Negative"
              }"
      )
