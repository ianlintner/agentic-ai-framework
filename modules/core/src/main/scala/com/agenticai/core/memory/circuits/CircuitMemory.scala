 ...
 * 
 * This provides a foundation for implementing the various circuit patterns 
 * from the video, like memory cells, clocks, shift registers, etc.
 */
object CircuitMemory {
   ...
   * 
   * @param initialValue The initial value stored in the cell
   ...
   * 
   * @param interval The number of ticks between pulses
   */
   ...
   * 
   * @param stages The number of stages in the register
   * @tparam T The type of value processed by the register
   */
  class ShiftRegister[T](stages: Int) {
    ...
    def get(stage: Int): Option[T] = {
      if (stage < 0 || stage >= stages) None else values(stage)
    }
   ...
   * @tparam T The type of value stored in the buffer
   */
