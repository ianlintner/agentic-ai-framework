 ...
 *
 * @tparam F The higher-kinded type
 */
trait Functor[F[_]] {
  /**
   ...
   * @return Mapped value in context F
   */
   ...
   * @return Value in context F with all positions replaced by b
   */
   ...
   * @return Function that operates on F[A]
   */
...
object Functor {
   ...
   *
   * @param mapF Implementation of map
  ...
  trait Laws[F[_]] {
    ...
    /**
     * Identity: map with the identity function is a no-op
     * map(fa)(identity) == fa
     */
