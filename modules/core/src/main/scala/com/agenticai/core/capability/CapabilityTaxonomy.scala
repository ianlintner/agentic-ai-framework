...
/** A structured taxonomy for agent capabilities.
  ...
  *   1. More precise capability matching 2. Capability inheritance (agents with a capability
  *      automatically have parent capabilities) 3. Capability relationships and compatibility
  *      detection
  */
object CapabilityTaxonomy:
  ...
  /** Represents a capability in the taxonomy.
    ...
    * @param parentId
    *   Optional parent capability ID (for hierarchy)
    * @param description
  ...
  trait CapabilityRegistry:
    ...
    /** Check if an agent with the given capabilities can fulfill a requirement. This handles
      * capability hierarchy, so an agent with a more specific capability can fulfill a requirement
      * for a more general capability.
      */
    def canFulfill(
  ...
  class InMemoryCapabilityRegistry extends CapabilityRegistry:
    ...
    override def canFulfill(
      ...
      if requiredCapabilities.isEmpty then ZIO.succeed(true)
      else if agentCapabilities.isEmpty then ZIO.succeed(false)
      else {
        // Direct capability check
        val directMatch = requiredCapabilities.subsetOf(agentCapabilities)
        if directMatch then ZIO.succeed(true)
        else
          ...
          ZIO
            .foreach(requiredCapabilities) { required =>
              ZIO
                .foreach(agentCapabilities) { agent =>
                  getParentCapabilities(agent).map(parents => parents.exists(_.id == required))
                }
