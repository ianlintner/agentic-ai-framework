...
/** A directory for composable agents that enables discovery and workflow creation.
  ...
  *   The capability taxonomy registry to use for capability relationships
  */
class ComposableAgentDirectory(capabilityRegistry: CapabilityRegistry):
  ...
  /** Register an agent in the directory
    ...
    * @return
    *   The unique ID assigned to the agent
    */
  ...
  /** Find agents that have all the required capabilities
    ...
    *   The set of capabilities that agents must have
    * @return
    *   A list of agents that satisfy the capabilities
    */
  def findAgentsByCapabilities(
      requiredCapabilities: Set[String]
  ): UIO[List[ComposableAgent[_, _]]] =
    if requiredCapabilities.isEmpty then
  ...
  /** Find agents by input and output types
    ...
    * @return
    *   A list of agents that match the types
    */
  ...
  /** Create a workflow by composing appropriate agents
    ...
    * @param requiredCapabilities
    *   Optional set of capabilities the workflow must provide
    * @return
    *   An optional composed agent forming the workflow
    */
  def createWorkflow[I, O](
    ...
    for
      ...
      capableDirectMatches <-
        if requiredCapabilities.isEmpty then ZIO.succeed(directMatches)
        else
          for
      ...
      workflow <-
        if capableDirectMatches.nonEmpty then
    ...
    findAgentsByCapabilities(requiredCapabilities).flatMap { capableAgents =>
      if capableAgents.isEmpty then
        ...
        if startAgents.isEmpty then
          ...
          if paths.isEmpty then ZIO.succeed(None)
          else {
            ...
            val composedAgent =
              composeAgentsInPath(shortestPath).asInstanceOf[ComposableAgent[I, O]]
  ...
  /** Find all possible paths from the starting agents to the target output type
    ...
    * @param visitedTypes
    *   Types already visited (to prevent cycles)
    * @return
    *   List of paths (each path is a list of agents)
    */
  private def findPaths(
    ...
    val intermediatePaths = startAgents.flatMap { agent =>
      ...
      if nextType == targetOutputType || visitedTypes.contains(nextType) then
  ...
  /** Compose a sequence of agents into a single agent
    *
    * @param agents
    *   The sequence of agents to compose
    * @return
    *   A composed agent
    */
  private def composeAgentsInPath(agents: List[ComposableAgent[_, _]]): ComposableAgent[?, ?] =
    agents.reduceLeft { (acc, next) =>
      acc
        .asInstanceOf[ComposableAgent[Any, Any]]
        .andThen(next.asInstanceOf[ComposableAgent[Any, Any]])
    }
...
object ComposableAgentDirectory:
  ...
  /** Create a new ComposableAgentDirectory
    *
    * @param capabilityRegistry
    *   The capability taxonomy registry to use
    * @return
    *   A new ComposableAgentDirectory instance
    */
