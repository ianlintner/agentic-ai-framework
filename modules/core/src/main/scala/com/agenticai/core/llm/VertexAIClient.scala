...
class VertexAIClient(config: VertexAIConfig):
  ...
  private lazy val settings = PredictionServiceSettings
    .newBuilder()
...
object VertexAIClient:
  ...
  /** Create a new VertexAIClient from an existing configuration
    */
  def create(config: VertexAIConfig): ZIO[Any, Throwable, VertexAIClient] =
    ZIO.attempt {
      // Validate credentials path if provided
      if !VertexAIConfig.validateCredentialsPath(config.credentialsPath) then
        throw new RuntimeException("Invalid credentials path")
