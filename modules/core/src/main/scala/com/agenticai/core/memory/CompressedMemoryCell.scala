...
/** GZIP compression strategy Uses java.util.zip.Deflater/Inflater with BEST_COMPRESSION level
  */
case object GzipCompressionStrategy extends CompressionStrategy:
  ...
  def decompress(data: Array[Byte]): Array[Byte] =
    ...
    while !inflater.finished() && shouldContinue do
      try
        val count = inflater.inflate(buffer)
        if count == 0 && inflater.needsInput() then
          shouldContinue = false // No more input and can't produce more output
        else if count > 0 then outputStream.write(buffer, 0, count)
      catch case _: Exception => shouldContinue = false
...
/** LZ4 compression strategy (faster but less compression) Note: Requires LZ4 library dependency
  */
  ...
  private def compressValue(value: A): ZIO[Any, MemoryError, CompressionStats] =
    ZIO
      .attempt {
        ...
        if shouldCompress(originalSize) then
...
object CompressedMemoryCell:
  ...
  def make[A](
    ...
    for
      ...
      size  = bytes.length
      _ <-
        if size < compressThreshold then
