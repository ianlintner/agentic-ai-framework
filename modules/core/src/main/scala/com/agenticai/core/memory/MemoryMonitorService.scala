...
object MemoryMonitorService {
  ...
  def makeComplete(
    ...
    for {
      ...
      _ <- ZIO.whenCase(sizeThreshold) {
        case Some(threshold) => service.setSizeThreshold(threshold)
      }
      _ <- ZIO.whenCase(countThreshold) {
        case Some(threshold) => service.setCountThreshold(threshold)
      }
    } yield service
  }
}