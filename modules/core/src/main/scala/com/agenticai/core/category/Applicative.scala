...
/**
 * Applicative typeclass, extending Functor with the ability to
 * wrap values and apply functions in the context.
 */
trait Applicative[F[_]] extends Functor[F] {
   ...
   * @return Value wrapped in the context F
   */
  def pure[A](a: A): F[A]
   ...
   *
   * @param ff Function in context F
   * @param fa Value in context F
   * @return Result of applying function to value, in context F
   */
  def ap[A, B](ff: F[A => B])(fa: F[A]): F[B]
  
  /**
   * Implementation of map in terms of pure and ap.
   * Default implementation is provided but can be overridden for efficiency.
   */
   ...
   *
   * @param fa First value in context F
   * @param fb Second value in context F
   * @return Combined values in context F
   */
   ...
   * @return Result of applying function to values, in context F
   */
   ...
   * @param fa First value in context F
   * @param fb Second value in context F
   * @param fc Third value in context F
   * @param f Ternary function to apply
   * @return Result of applying function to values, in context F
   */
...
object Applicative {
   ...
   *
   * @param pureF Implementation of pure
   * @param apF Implementation of ap
  ...
  trait Laws[F[_]] {
    ...
    /**
     * Identity: ap with a pure identity function is a no-op
     * ap(pure(identity))(fa) == fa
     */
    def identityLaw[A](fa: F[A]): Boolean = {
      val identityF = F.pure[A => A](identity)
      val result = F.ap(identityF)(fa)
      result == fa
    }
    
    /**
     * Homomorphism: applying a pure function to a pure value is the same as applying the function to the value and then using pure
    ...
    /**
     * Interchange: applying a function in a context to a pure value is the same as applying pure to that value and then applying it to the function
    ...
    /**
     * Composition: composing functions in a context is the same as composing the functions and then using pure
     * ap(ap(ap(pure(compose))(u))(v))(w) == ap(u)(ap(v)(w))
     * where compose = (f: B => C) => (g: A => B) => (a: A) => f(g(a))
     */
