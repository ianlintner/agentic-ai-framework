...
class PersistentMemorySystem(baseDir: File, runtime: Runtime[Any]) extends MemorySystem:
  ...
  if !baseDir.exists() then baseDir.mkdirs()
  ...
  override def createCell[T](name: String): ZIO[Any, Throwable, MemoryCell[T]] =
    for
      ...
      id   = if name.contains("/") then name else java.util.UUID.randomUUID().toString
    ...
    for
      ...
      _ <- ZIO.foreachDiscard(cellOpt) { cell =>
        ...
        for
          ...
          _ <- ZIO.succeed {
            tags.foreach(tag => tagIndex.update(tag, tagIndex.getOrElse(tag, Set.empty) - name))
          }
        yield ()
      }
      ...
      _ <- ZIO.succeed {
        ...
        if file != null && file.exists() then file.delete()
      }
    yield ()
  ...
  def runCleanup(strategy: CleanupStrategy): ZIO[Any, MemoryError, Int] =
    for
      allCells <- getAllCells.mapError(e =>
        MemoryError.ReadError(s"Error getting cells: ${e.getMessage}")
      )
      cellsToCleanup <- ZIO.filter(allCells.toList)(cell =>
        strategy
          .shouldCleanup(cell)
          .mapError(e => MemoryError.ReadError(s"Error checking cleanup: ${e.getMessage}"))
      )
      ...
      Unsafe.unsafe { implicit unsafe =>
        ...
        val writeOpResult = currentValue match
          case Some(value) =>
            ...
            try runtime.unsafe.run(writeOp).getOrThrow()
            catch case e: Exception => println(s"Error writing value to cell: ${e.getMessage}")
        ...
        val proxy = new MemoryCell[Any]:
          ...
          override def update(f: Option[Any] => Any) =
            ...
            innerCell
              .asInstanceOf[MemoryCell[Any]]
              .update(typedF)
              .flatMap(_ => saveCell(id, innerCell.asInstanceOf[MemoryCell[Any]]))

          override def metadata = innerCell.metadata
          override def clear =
            innerCell.clear.flatMap(_ => saveCell(id, innerCell.asInstanceOf[MemoryCell[Any]]))
          override def empty =
            innerCell.empty.flatMap(_ => saveCell(id, innerCell.asInstanceOf[MemoryCell[Any]]))
          override def getMetadata = innerCell.getMetadata
          override def addTag(tag: String) = innerCell
            .addTag(tag)
            .flatMap(_ => saveCell(id, innerCell.asInstanceOf[MemoryCell[Any]]))
          override def removeTag(tag: String) = innerCell
            .removeTag(tag)
            .flatMap(_ => saveCell(id, innerCell.asInstanceOf[MemoryCell[Any]]))
